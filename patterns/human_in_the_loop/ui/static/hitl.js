...
IMPORTANT: Return ONLY the press release text. Do not include any conversational preamble like "Here is a draft" or "Sure". Start directly with the headline or dateline.`;
    ...
    submitFeedback() {
        ...
        if (val) {
            ...
            if (this.state.waitingForConfirmation) {
               ...
               this.sendMessage(`I don't like this draft. Change this: ${val}. Regenerate the draft.`);
            }
            input.value = "";
        }
    },
    ...
    async sendMessage(text, statusText = "Processing...") {
        ...
        // We do NOT add user message to UI in document view mode.
        ...
        try {
            ...
            if (data.history && data.history.length > 0) {
                ...
                if (lastMsg.role !== "user") {
                    ...
                    // State Detection
                    // 1. Success
                    if (contentLower.includes("success") || contentLower.includes("published")) {
                        // this.updateDocument("Process Complete.", true); // Don't wipe doc
                        this.state.waitingForConfirmation = false;
                        // Maybe show a success indicator on the button? or toast?
                        const btn = document.getElementById("btn-generate");
                        if (btn) btn.textContent = "Published! ✅";
                    } 
                    // 2. Confirmation (ADK Native)
                    else if (contentLower.includes("confirm") || contentLower.includes("approve") || contentLower.includes("proceed")) {
                         this.state.waitingForConfirmation = true;
                         this.showControls();
                    }
                    // 3. Draft Review (Normal response)
                    else {
                         this.state.waitingForConfirmation = false;
            ...
            // Only reset loading if we didn't just mistakenly overwrite "Published!" 
            // Actually, setLoading(false) reverts to "Draft Press Release". 
            // If published, we might want to keep it? 
            // For now, standard behavior:
            const btn = document.getElementById("btn-generate");
            if (btn && !btn.textContent.includes("Published")) {
               this.setLoading(false);
            } else if (btn) {
               // Enable it back after a delay?
               btn.disabled = false;
            }
        }
    },
